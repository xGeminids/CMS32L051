...
uint8_t tx_buf[] = "UART2(IrDA) Send and Reveive is DONE!";
uint8_t rx_buf[37];
...
//-----------------------------------------------------------------------
// IrDA(UART2) transmission/reception
//-----------------------------------------------------------------------
	printf("Please connect No.31 pin(P13/IrTxd) to No.30 pin(P14/IrRxd)\n");
	printf("Press any key continue...\n");
	getchar();
	
	UART2_Init(SystemCoreClock, 19200);
	UART2_Start();
	UART2_Receive(rx_buf, sizeof(rx_buf));
	UART2_Send(tx_buf, sizeof(tx_buf));
