...
MD_STATUS UART_BaudRateCal(uint32_t fclk_freq, uint32_t baud, uart_baud_t *pvalue)
{
    ...
    for (i = 0; i < 16; i++)
    {
        fmck_freq = (double)(fclk_freq) / pow(2, i);
        for (j = 2; j < 128; j++)
        {
            baud_cal = fmck_freq / (j + 1) / 2;
            baud_err = baud_cal / baud * 100 - 100;
            if (fabs(baud_err) < 0.2)
            {
                pvalue->prs = i;
                pvalue->sdr = j;
                //printf("fclk_freq = %10dHz, baud = %6dbps, prs = %2d, sdr = %3d, errors = %3.2f%%\n", fclk_freq, baud, pvalue->prs, pvalue->sdr, baud_err);
                status = MD_OK;
                return (status);
            }
        }
...
MD_STATUS UART1_Init(uint32_t freq, uint32_t baud)
{
    ...
    NVIC_DisableIRQ(ST1_IRQn);       /* disable INTST1 interrupt flag */
    NVIC_ClearPendingIRQ(ST1_IRQn);  /* clear INTST1 interrupt flag */
    NVIC_DisableIRQ(SR1_IRQn);       /* disable INTSR1 interrupt flag */
    NVIC_ClearPendingIRQ(SR1_IRQn);  /* clear INTSR1 interrupt flag */
    NVIC_DisableIRQ(SRE1_IRQn);      /* disable INTSRE1 interrupt flag */
    NVIC_ClearPendingIRQ(SRE1_IRQn); /* clear INTSRE1 interrupt flag */
...
void SPI11_MasterInit(spi_mode_t mode)
{
	...
	//SCI0->SDR03 = 5 << 9;		// fMCK/(1+n)x2 = fMCK/12 以下分频可以稳定传输
...
MD_STATUS UART2_Init(uint32_t freq, uint32_t baud)
{
    ...
    SCI1->ST1 |= _0002_SCI_CH1_STOP_TRG_ON | _0001_SCI_CH0_STOP_TRG_ON;
    NVIC_DisableIRQ(ST2_IRQn);       /* enable INTST2 interrupt flag */
    NVIC_ClearPendingIRQ(ST2_IRQn);  /* clear INTST2 interrupt flag */
    NVIC_DisableIRQ(SR2_IRQn);       /* enable INTSR2 interrupt flag */
    NVIC_ClearPendingIRQ(SR2_IRQn);  /* clear INTSR2 interrupt flag */
    NVIC_DisableIRQ(SRE2_IRQn);      /* enable INTSRE2 interrupt flag */
    NVIC_ClearPendingIRQ(SRE2_IRQn); /* clear INTSRE2 interrupt flag */
...
void UART2_Start(void)
{
    ...
    NVIC_EnableIRQ(ST2_IRQn);       /* enable INTST2 interrupt */
    NVIC_EnableIRQ(SR2_IRQn);       /* enable INTSR2 interrupt */
}
/***********************************************************************************************************************
...
MD_STATUS SPI21_MasterSendReceive(uint8_t *const tx_buf, uint16_t tx_num, uint8_t *const rx_buf)
{
    ...
    else
    {
        ...
        DMAVEC->CTRL[CTRL_DATA_SPI21].DMSAR = (uint32_t)&SCI1->SIO10;
...
void SPI21_SlaveInit(spi_mode_t mode)
{
    ...
    ckp = (~mode & 0x02) >> 1;
    CGC->PER0 |= CGC_PER0_SCI1EN_Msk;
    SCI1->ST1 |= _0004_SCI_CH2_STOP_TRG_ON;
    SCI1->SIR11 = _0004_SCI_SIRMN_FECTMN | _0002_SCI_SIRMN_PECTMN | _0001_SCI_SIRMN_OVCTMN;
...
{
    CGC->PER0 |= CGC_PER0_SCI1EN_Msk;
    SCI1->SPS1 = _0020_SCI_CK01_fCLK_2 | _0000_SCI_CK00_fCLK_0;
