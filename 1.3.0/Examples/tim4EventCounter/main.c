...
//-----------------------------------------------------------------------
	...
	PORT->PM0  &= ~(1 << 0);    /* P00 is output */
	
	for(i=1; i<30; i++)
	{
		PORT->PSET0 = 1 << 0;  // TI00 input High level
		delayMS(1);
		PORT->PCLR0 = 1 << 0;  // TI00 input Low level
		delayMS(1);
...
#else	
	TM40_EventCount(TM4_CHANNEL_1, TM4_CH1_INPUT_TI01, TM4_TI_POSEDGE, 10);
	PORT->PM1  &= ~(1 << 6);    /* P16 is output */
	for(i=1; i<30; i++)
	{
		PORT->PSET1 = 1 << 6;  // TI01 input High level
		delayMS(1);
		PORT->PCLR1 = 1 << 6;  // TI01 input Low level
		delayMS(1);
		printf("No.%3d, TCR01 = %03d, IntTaken = %d\n", i, TM40->TCR01, INTC_GetPendingIRQ(TM01_IRQn));
