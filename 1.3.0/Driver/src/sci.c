...
#if defined USE_SCI_UART0_RX && defined USE_SCI_UART0_TX
/***********************************************************************************************************************
...
MD_STATUS UART0_Init(uint32_t freq, uint32_t baud)
{
    ...
    SCI0->SMR00 = _0020_SMRMN_DEFAULT_VALUE | _0000_SCI_CLOCK_SELECT_CK00 | _0000_SCI_CLOCK_MODE_CKS |
                  _0002_SCI_MODE_UART | _0001_SCI_BUFFER_EMPTY;//_0000_SCI_TRANSFER_END;
...
MD_STATUS UART0_BaudRate(uint32_t fclk_freq, uint32_t baud)
{
    ...
    if (status == MD_OK)
    {
        ...
        SCI0->SPS0 |= SCI0_SPS0_PRS00_Msk & (pvalue.prs << SCI0_SPS0_PRS00_Pos);
...
* @brief  This function starts the UART0 module operation.
* @param  None
* @return None
***********************************************************************************************************************/
...
MD_STATUS UART0_Send(uint8_t *const tx_buf, uint16_t tx_num)
{
    ...
    else
    {
        ...
        DMA_Start(DMA_VECTOR_ST0, CTRL_DATA_UART0, DMA_MODE_NORMAL, DMA_SIZE_BYTE, tx_num, tx_buf, (uint16_t *)&SCI0->TXD0);
...
#if defined (USE_SCI_UART1_RX) && defined (USE_SCI_UART1_TX)
/***********************************************************************************************************************
...
MD_STATUS UART1_BaudRate(uint32_t fclk_freq, uint32_t baud)
{
    ...
    if (status == MD_OK)
    {
        SCI0->ST0 = _0008_SCI_CH3_STOP_TRG_ON | _0004_SCI_CH2_STOP_TRG_ON;
        SCI0->SPS0 &= ~SCI0_SPS0_PRS01_Msk;
        SCI0->SPS0 |= SCI0_SPS0_PRS01_Msk & (pvalue.prs << SCI0_SPS0_PRS01_Pos);    
...
MD_STATUS UART1_Send(uint8_t *const tx_buf, uint16_t tx_num)
{
    ...
    else
    {
        ...
        DMA_Start(DMA_VECTOR_ST1, CTRL_DATA_UART1, DMA_MODE_NORMAL, DMA_SIZE_BYTE, tx_num, tx_buf, (uint16_t *)&SCI0->TXD1);
        DMA_Trigger(DMA_VECTOR_ST1);
        #endif
    }
...
void SPI10_MasterInit(spi_mode_t mode)
{
    ...
    SCI0->SDR02 = _9E00_SCI_BAUDRATE_DIVISOR;//_CE00_SCI_BAUDRATE_DIVISOR;
    /* Set output enable */
    if ((mode == SPI_MODE_0) || (mode == SPI_MODE_1))
    {
...
void SPI11_MasterInit(spi_mode_t mode)
{
    ...
    SCI0->SIR03 = _0004_SCI_SIRMN_FECTMN | _0002_SCI_SIRMN_PECTMN | _0001_SCI_SIRMN_OVCTMN;
    SCI0->SMR03 = _0020_SMRMN_DEFAULT_VALUE | _8000_SCI_CLOCK_SELECT_CK01 | _0000_SCI_CLOCK_MODE_CKS |
                  _0000_SCI_TRIGGER_SOFTWARE | _0000_SCI_MODE_SPI | _0001_SCI_BUFFER_EMPTY;//_0000_SCI_TRANSFER_END;
...
#if defined USE_SCI_UART2_RX && defined USE_SCI_UART2_TX
/***********************************************************************************************************************
...
MD_STATUS UART2_BaudRate(uint32_t fclk_freq, uint32_t baud)
{
    ...
    {
        SCI1->ST1 = _0002_SCI_CH1_STOP_TRG_ON | _0001_SCI_CH0_STOP_TRG_ON;
        SCI1->SPS1 &= ~SCI1_SPS1_PRS10_Msk;
        SCI1->SPS1 |= SCI1_SPS1_PRS10_Msk & (pvalue.prs << SCI1_SPS1_PRS10_Pos);
...
MD_STATUS UART2_Send(uint8_t *const tx_buf, uint16_t tx_num)
{
    ...
    else
    {
        ...
        DMA_Start(DMA_VECTOR_ST2, CTRL_DATA_UART2, DMA_MODE_NORMAL, DMA_SIZE_BYTE, tx_num, tx_buf, (uint16_t *)&SCI1->TXD2);
        DMA_Trigger(DMA_VECTOR_ST2);
        #endif
    }
